...
PB.est.2.func = function(mu1.data.2.in, mu2.data.2.in, mu3.data.2.in, 
  ...
  mu12.boot.2 = sapply(1:n.B.in, function(x2){
    ...
    return(PB.est.1.func(mu1.boot.2, mu2.boot.2, mu3.boot.2, n.in, n.B.in))
  })
...
## parametric bootstrap triple
PB.est.3.func = function(mu1.data.3.in, mu2.data.3.in, mu3.data.3.in, 
                         n.in, n.B.in){
  mu12.boot.3 = sapply(1:n.B.in, function(x3){
    mu1.boot.3 = rnorm(n.in, mean(mu1.data.3.in), sd = sd(mu1.data.3.in))
    mu2.boot.3 = rnorm(n.in, mean(mu2.data.3.in), sd = sd(mu2.data.3.in)) 
    mu3.boot.3 = rnorm(n.in, mean(mu3.data.3.in), sd = sd(mu3.data.3.in)) 
    
    return(PB.est.2.func(mu1.boot.3, mu2.boot.3, mu3.boot.3, n.in, n.B.in))
  })
  return(2*PB.est.2.func(mu1.data.3.in, mu2.data.3.in, mu3.data.3.in,
                         n.in, n.B.in)-
           mean(mu12.boot.3))
}
...
NB.est.2.func = function(mu1.data.2.in, mu2.data.2.in, mu3.data.2.in, 
  ...
  mu12.boot.2 = sapply(1:n.B.in, function(x2){
    ...
    return(NB.est.1.func(mu1.boot.2, mu2.boot.2, mu3.boot.2, n.in, n.B.in))
  })
...
## non-parametric bootstrap triple
NB.est.3.func = function(mu1.data.3.in, mu2.data.3.in, mu3.data.3.in, 
                         n.in, n.B.in){
  mu12.boot.3 = sapply(1:n.B.in, function(x3){
    mu1.boot.3 = sample(mu1.data.3.in, n.in, replace = TRUE)
    mu2.boot.3 = sample(mu2.data.3.in, n.in, replace = TRUE)
    mu3.boot.3 = sample(mu3.data.3.in, n.in, replace = TRUE)
    
    return(NB.est.2.func(mu1.boot.3, mu2.boot.3, mu3.boot.3, n.in, n.B.in))
  })
  return(2*NB.est.2.func(mu1.data.3.in, mu2.data.3.in, mu3.data.3.in,
                         n.in, n.B.in)-
           mean(mu12.boot.3))
}

## Jackknife
JK.est.func = function(mu1.data.1.in, mu2.data.1.in, mu3.data.1.in, n.in){
  ...
  mu12.JK = sapply(1:n.in, function(x1.in){
    ...
    return(max(mu1.sample.2, mu2.sample.2, mu3.sample.2))
  })
  
  jack.1 = n.in*max(mean(mu1.data.1.in), mean(mu2.data.1.in), 
                 mean(mu3.data.1.in)) - (n.in-1)*mean(mu12.JK)
  
  return(jack.1)
}

## Lindley's shrinkage estimator
est.L.func = function(mu1.data.L.in, mu2.data.L.in, mu3.data.L.in, n.in){
  ...
  sigma2.func = mean(var(mu1.data.L.in), var(mu2.data.L.in),
                     var(mu3.data.L.in))
  C.func = max(0, 1-sigma2.func/(n.in*((mean(mu1.data.L.in)-Xbar.func)^2+
...
## Hwang's shrinkage estimator
est.H.func = function(mu1.data.L.in, mu2.data.L.in, mu3.data.L.in, n.in){
  XS.func = max(mean(mu1.data.L.in), mean(mu2.data.L.in), mean(mu3.data.L.in))
  mu.prior = 1
  tau.prior = 100
  sigma2.func = mean(var(mu1.data.L.in), var(mu2.data.L.in),
                     var(mu3.data.L.in))
  C.func = 1-sigma2.func/(n.in*tau.prior + sigma2.func)
  # C.func = 0
  return(C.func*XS.func + (1-C.func)*mu.prior)
}
